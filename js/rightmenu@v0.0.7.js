function setMask() {
    return null != document.getElementsByClassName("rmMask")[0] ? document.getElementsByClassName("rmMask")[0] : (mask = document.createElement("div"),
...
function insertAtCursor(e, t) {
    ...
    sel.text = t,
    sel.select()) : e.selectionStart || "0" == e.selectionStart ? (n = e.selectionStart,
    o = e.selectionEnd,
    s = e.scrollTop,
    e.value = e.value.substring(0, n) + t + e.value.substring(o, e.value.length),
    0 < s && (e.scrollTop = s),
    e.focus(),
    e.selectionStart = n + t.length,
    e.selectionEnd = n + t.length) : (e.value += t,
    e.focus())
}
function downloadImageAsBlob(n) {
    fetch(n).then(e => e.blob()).then(e => {
        var t = window.URL.createObjectURL(e)
          , e = document.createElement("a");
        e.style.display = "none",
        e.href = t,
        e.download = n.split("/").pop(),
...
function showLinkMenu(e) {
    $(".menu-to").show(),
    rmf.open = function() {
        -1 == e.indexOf("http://") && -1 == e.indexOf("https://") || -1 != e.indexOf(location.host) ? pjax.loadUrl(e) : location.href = e
    }
    ,
...
function showImgMenu(e) {
    $(".menu-img").show(),
    rmf.openImageInNewTab = function() {
        window.open(e)
    }
    ,
    rmf.copyImageLink = async function() {
        try {
            await navigator.clipboard.writeText(e),
            Snackbar.show({
                text: "图片链接复制成功！",
...
function popupMenu() {
    window.oncontextmenu = function(e) {
        ...
        "A" == el.tagName ? showLinkMenu(el.href) : "A" == el.parentNode.tagName && ("IMG" == el.tagName && el.src == el.parentNode.href || showLinkMenu(el.parentNode.href)),
        "IMG" == el.tagName ? showImgMenu(el.src) : "TEXTAREA" != el.tagName && "INPUT" != el.tagName || ($(".menu-paste").show(),
        rmf.paste = function() {
        ...
        let t = e.clientX + 10
          , n = e.clientY;
        var o = $("#rightMenu").width()
          , e = $("#rightMenu").height();
        return t + o > window.innerWidth && (t -= o + 10),
        n + e > window.innerHeight && (n -= n + e + 10 - window.innerHeight),
        mask = setMask(),
        window.onscroll = () => {
            rmf.showRightMenu(!1),
            window.onscroll = () => {}
...
,
rmf.copyPageLink = async function() {
    try {
...
rmf.searchinThisPage = function() {
    ...
    e.initEvent("input", !1, !1),
    document.getElementsByClassName("DocSearch-Input")[0].dispatchEvent(e)
}
,
